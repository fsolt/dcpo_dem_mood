...
if (!file.exists(here("data",
    ...
    osf_retrieve_file("rfamx") %>% 
        osf_download(here("data"))
}
if (!file.exists(here("data",
                      "dcpo_ajps.rda"))) { 
    osf_retrieve_file("sbp5a") %>% 
        osf_download(here("data"))
}
if (!file.exists(here("data",
                      "dcpo_apsr.rda"))) { 
    osf_retrieve_file("9yh5z") %>% 
        osf_download(here("data"))
}
...
exp_claassen_input <- rio::import(here("data", "exp_claassen_input.rds")) 
load(here("data", "dcpo_ajps.rda"))
load(here("data", "dcpo_apsr.rda"))

## Preloading chunk-produced results to speed up the document compiling
# Download from osf if files not present 
# (can also be generated by compiling the appropriate eval=FALSE chunks below)
if (!file.exists(here("output",
                      "estimates_clsMeanAJPS.RDS"))) { 
    osf_retrieve_file("s36h2") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_clsMeanAPSR.RDS"))) { 
    osf_retrieve_file("cuj63") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_correctAJPS.RDS"))) { 
    osf_retrieve_file("nbt4j") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_expcorAJPS.RDS"))) { 
    osf_retrieve_file("hd874") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_correctAPSR.RDS"))) { 
    osf_retrieve_file("fv3qz") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_expcorAPSR.RDS"))) { 
    osf_retrieve_file("td5nr") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_dcpoAJPS.RDS"))) { 
    osf_retrieve_file("8avpt") %>% 
        osf_download(here("output"))
}
if (!file.exists(here("output",
                      "estimates_moc_dcpoAPSR.RDS"))) { 
    osf_retrieve_file("e2ys3") %>% 
        osf_download(here("output"))
}
...
dcpo_input_nc <- exp_claassen_input$mar$data %>% 
  dplyr::select(country) %>% 
  distinct() %>% 
  nrow()

dcpo_input_surv <- exp_claassen_input$mar$data %>%
...
dcpo_input_ncy <- exp_claassen_input$mar$data %>% 
  dplyr::select(country, year) %>% 
  distinct() %>% 
  nrow()

dcpo_input_ncyi <- exp_claassen_input$mar$data %>% 
...
increase_cy <- sprintf("%2.1f", (dcpo_input_ncy/supdem_ncy - 1) * 100)
increase_cyi <- sprintf("%2.1f", (dcpo_input_ncyi/supdem_ncyi - 1) * 100)
increase_surv <- sprintf("%2.1f", (dcpo_input_surv/supdem_surv - 1) * 100)
```
...
We employed `r dcpo_input_ncyi %>% formatC(big.mark=",")` national opinions on democracy from `r dcpo_input_surv %>% formatC(big.mark=",")` national surveys, representing a `r increase_cyi`% and `r increase_surv`% increase respectively over the `r supdem_ncyi %>% formatC(big.mark=",")` opinions and `r supdem_surv %>% formatC(big.mark=",")` national surveys used in Claassen [-@Claassen2020a; -@Claassen2020].^[
...
if (!file.exists(here("data", "Support_democracy_ajps.csv"))) {
  tempfile <- get_file_by_doi(filedoi = "10.7910/DVN/HWLW0J/MVXNFC")
  
  writeBin(tempfile, here("data", "Support_democracy_ajps.csv"))
}
...
tidy_cls <- map(df_result_cls, function(result) {
    ...
    if (any(str_detect("plm", class(result)))) {
...
glance_cls <- map(df_result_cls, function(aResult) {
    if (any(str_detect("plm", class(aResult)))) {
