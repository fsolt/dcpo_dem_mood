...
available <- anti_join(claassen_input_cy, supdem_cy)                # 487 obs
...
cys_to_drop <- anti_join(available, year_fixes, by = c("country", "year" = "year.y", "project")) %>% # 398 obs
    select(-y_dcpo)
...
load("../data/claassen_m5_theta.rda")
...
df_controls <- read_csv("../data/Support_democracy_ajps.csv")
...
result_betas_m5 <- MIextract(result_m5, fun = coef)
result_vars_m5 <- MIextract(result_m5, fun = vcovBK_time)

summary(result_m5O)
summary(MIcombine(results = result_betas_m5, 
