...
mocdcpo_APSR <- readRDS(here("ouput","estimates_moc_dcpoAPSR.RDS"))
...
It is worth to note that, despite the great effort of collecting data and identifying differences, we could not reach a completely primary replication of @Claassen2020a's Dataverse materials.
...
Nevertheless, given there are over 2000 observations involved in the analysis, the five missing should not make a meaningful difference.
...
We conduct inferences from the distributive data via a technique known as "method of composition" (MOC) [@Tanner1996, p.52;@Treier2008, p. 215; @caughey2018,p.A15-16].
See more details of the method and the operational process in Online Supplementary Material \@ref(moc).
...
names(mocdcpo_APSR) <- c("pooled1", "pooled-regime1", "pooled2", "pooled-regime2",
                                "fd1", "fd-regime1", "fd2", "fd-regime2")


saveRDS(mocdcpo_APSR, file = here::here("ouput","estimates_moc_dcpoAPSR.RDS"))
```
...
result_ecm1 <- bind_rows(
  mutate(result_clsAPSR[["pooled1"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["pooled1"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["pooled1"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "ECM")
...
result_fd1 <- bind_rows(
  mutate(result_clsAPSR[["fd1"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["fd1"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["fd1"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "First Difference")
...
result_APSR1 <- bind_rows(result_ecm1, result_fd1) %>%
  ...
  mutate(
    ...
    type = factor(type,  levels = c("ECM", "First Difference")),
    model = factor(model, levels = c("Original","Uncertianty+ExpData", "Uncertianty+DCPO+ExpData"))
  )
...
result_ecmReg1 <- bind_rows(
  mutate(result_clsAPSR[["pooled-regime1"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["pooled-regime1"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["pooled-regime1"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "ECM")
...
result_fdReg1 <- bind_rows(
  mutate(result_clsAPSR[["fd-regime1"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["fd-regime1"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["fd-regime1"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "First Difference")
...
result_ecm2 <- bind_rows(
  mutate(result_clsAPSR[["pooled2"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["pooled2"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["pooled2"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "ECM")

result_fd2 <- bind_rows(
  mutate(result_clsAPSR[["fd2"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["fd2"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["fd2"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "First Difference")
...
result_ecmReg2 <- bind_rows(
  mutate(result_clsAPSR[["pooled-regime2"]][[1]][,1:3], model = "Original"),
 mutate(MOCsumm2(mocresult_expAPSR[["pooled-regime2"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["pooled-regime2"]]), model = "Uncertianty+DCPO+ExpData")) %>%
  mutate(type = "ECM")

result_fdReg2 <- bind_rows(
  mutate(result_clsAPSR[["fd-regime2"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm2(mocresult_expAPSR[["fd-regime2"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm2(mocdcpo_APSR[["fd-regime2"]]), model = "Uncertianty+DCPO+ExpData")) %>% 
  mutate(type = "First Difference")
...
type_model <-
    c("Original", "Uncertianty+ExpData", "Uncertianty+DCPO+ExpData")
...
result_pooled <- bind_rows(
  mutate(result_clsAJPS[["pooled"]][[1]][,1:3], model = "Original"),
 mutate(MOCsumm(mocresult_expAJPS[["pooled"]]), model = "Uncertianty+ExpData"),
 mutate(MOCsumm(mocresult_dcpoAJPS[["pooled"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "Pooled")

result_gmm <- bind_rows(
  mutate(result_clsAJPS[["gmm"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm(mocresult_expAJPS[["gmm"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm(mocresult_dcpoAJPS[["gmm"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "GMM")
...
result_regime <- bind_rows(
  mutate(result_clsAJPS[["pooled-regime"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm(mocresult_expAJPS[["pooled-regime"]]), model = "Uncertianty+ExpData"),
  mutate(MOCsumm(mocresult_dcpoAJPS[["pooled-regime"]]), model = "Uncertianty+DCPO+ExpData")
) %>% 
  mutate(type = "Pooled")

result_regimeGmm <- bind_rows(
  mutate(result_clsAJPS[["gmm-regime"]][[1]][,1:3], model = "Original"),
  mutate(MOCsumm(mocresult_expAJPS[["gmm-regime"]]), model = "Uncertianty+ExpData"),
 mutate(MOCsumm(mocresult_dcpoAJPS[["gmm-regime"]]), model = "Uncertianty+DCPO+ExpData")
)%>% 
  mutate(type = "GMM")
...
# Replication with MOC Accounting for Uncertainty {#moc}
...
Since measurement uncertainty associated with these latent variables can propogate into the inferences over coefficient parameters in models, we incorporate uncertainty by employing "Method of Composition" [@Tanner1996,p52], which has been applied in analyses with latent variables [@treier2008;@caughey2018;@Kastellec2015].

As @caughey2018 explained, the main idea of MOC is to estimate the marginal distribution of coefficient parameter vector $\beta$, intergrating over the uncertainty in latent variables $\theta$ (P A-15).
...
In this way, we incorportate uncertainty in measuring predictor $\theta$, and uncertainty in the effects of latent variables $\theta$ and other variables $w$ on outcome variable $y$ [@treie2008, 215].
...
We calcluate point estimates and standard error based on these 900 samples.
