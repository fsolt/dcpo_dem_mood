...
index_coefName <- c(
  ...
  `plm::lag(theta_trim, 1)` = "Support[t-1]",
...
type_model <-
  c("Claassen 2019", "Claassen Uncertainty", "DCPO Uncertainty")

result_ajpsPooled <-
  list(result_cls,
       result_clsUncertainFull,
       result_dcpoUncertainFull) %>%
  map2_df(type_model, ~ {
    .x$pooled[[1]] %>%
...
saveRDS(result_clsFullUncertainAPSR, file = here("output", "estimates_clsFullUncertainAPSR.RDS"))
```
...
saveRDS(result_dcpoUncertainAPSR, file = here("output", "estimates_dcpoUncertainAPSR.RDS"))
```
...
result_dcpoUncertainAPSR <- readRDS(here("output", "estimates_dcpoUncertainAPSR.RDS"))
...
result_apsrPooled <-
  list(result_clsAPSR,
       result_clsFullUncertainAPSR,
       result_dcpoUncertainAPSR) %>%
  map2_df(type_model, ~ {
    .x$pooled1[[1]] %>%
      filter(term != "(Intercept)") %>%
      select(term, estimate, conf.low, conf.high) %>%
...
```{r num-cls2020aUncertain, results='asis'}
...
```{r num-cls2020aDCPO, results='asis'}
...
```{r num-cls2020bUncertain, results='asis'}
...
```{r num-cls2020bDCPO, results='asis'}
...
result_dcpoUncertainAPSR <- readRDS(here("output", "estimates_dcpoAPSR.RDS"))
...
result_ecm1 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$pooled1[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fd1 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$fd1[[1]], model = "DCPO")
) %>% 
...
result_ecm2 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$pooled2[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fd2 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$fd2[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
result_ecmReg1 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$`pooled-regime1`[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fdReg1 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$`fd-regime1`[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
result_ecmReg2 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$`pooled-regime2`[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fdReg2 <- bind_rows(
  ...
  mutate(result_dcpoUncertainAPSR$`fd-regime2`[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
df_dcpo <- purrr::map(1:900, function(anEntry) {
  dcpo_ajps[[anEntry]] %>%     #original data has include theta_dem_trim
    plm::pdata.frame(index = c("country", "year"))
})
