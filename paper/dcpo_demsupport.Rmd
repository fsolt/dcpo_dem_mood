...
One of the classic but disputable questions in comparative politics is whether mass democratic support stabilizes democracy.
...
However, it is questionable that whether self-report attitude toward democracy could transfer to behaviorally democratic commitments when the democratic system is in economic or societal crisis.
...
However, one more essential concern is whether and how we can measure the latent public opinion correctly, especially for comparative public opinion survey data.
...
Public support for democracy is a latent variable and the incorrect measurement of it may conceal the real relationship between public opinion and democratic insitution development.
...
To adjudicate/contribute to the ongoing debate, this research note reexamines the relationship between public democratic support and democracy stability by applying a superior measure and incorporating a larger data and uncertainty.
We use a superior measure, Dynamic Comparative Public Opinion (DCPO) model developed by @Solt2020a, to estimate democratic support for a larger data with 144 countries for up to 32 years between 1988 and 2019. 
In addition to account for item response bias across countries and the extent of polarization by generating estimates on bounded scales, the DCPO model has superiority in dealing with both binominal and ordinal survey data cross countries over time. 
...
Next, we incorporate uncertainty by using random draws from the posterior distributions and following @Rubin1987's rules.
A latent construct should be measured with uncertainty because it is impossible to measure the complex concept perfectly and uncertainty is substantively necessary for “comparing relationship between complex theoretical concepts across political contexts and over time" [@Crabtree2015,p2].

Surprisingly, our study reveals that there is no significant relationship between public support and democratic change in both the replication analysis of @Claassen2020a model and DCPO analysis when uncertainty is included.
...
# More Data
We collected available data from 62 survey questions in 120 surveys across 159 countries for up to 33 years between 1988 and 2020.
Survey data was extracted automatically through DCPO package.
DCPO package also automatically identifies survey conducted year, which captures more accurate correspondence between public opinion and democracy level.
Compared to Claassen's (2019) 1178 available country-years, our data consists of 1406 country-years, which is a 19.4% increase.
The mass support for democracy is estimated for 144 countries with at least 2 country-years observed.
Since V-Dem does not have data for small countries and data for 2020, xx countries are filtered from our final data set.
The dataset in our analysis consists of xxxx estimates, drawn from 144 countries. 
...
The DCPO shows its superiority in several significant ways. 
...
Results from validation tests in @Solt2020a.
In validation tests, @Solt2020a employs k-fold validation with 10 folds, which reduces the misrepresentation of the model due to the chosen data from single cross-validation.
The validation tests corroborate that DCPO not only features the smallest mean absolute error and largest improvement over the country-means MAE but also provides a more stable and efficient prediction on out-of-sample data than either of @Claassen2019 Model 5 and @Caughey2019 model.
...
Instead of simply employing the point estimates, that is, the means of the posterior distributions [_contra_ @Claassen2020a; @Claassen2020b], it is recommended to incorporate uncertainty in using latent variables [e.g., @Schnakenberg2014; @Crabtree2015] and other measures, such as the Standardized World Income Inequality Database [@Solt2020].
In this study, we generate 1,000 duplicate versions of the analysis dataset, assign to each a different random draw from the posterior distributions of public democratic support, perform the analyses repeatedly on each of these 1,000 versions of the dataset, and combine the results following the rules set out in @Rubin1987.
...
save(df_result_cls, file = here::here("output", "estimates_clsAJPS.RDS"))
```
...
```{r dcpoUncertaintyAJPS, eval = FALSE}
...
df_result_dcpo <- map(mod_ls, estimate_dcpo)

save(df_result_dcpo, file = here::here("output", "estimates_dcpo.RDS"))
```

```{r visualizedMood, fig.cap= "The Effect of Democratic Mood on Institutional Democratization"}

load(here::here("output", "estimates_clsAJPS.RDS"))
load(here::here("output", "estimates_dcpoMeanAJPS.RDS"))
load(here::here("output", "estimates_dcpo.RDS"))

df_plot <- df_result_cls[[1]] %>% 
  mutate(model = "Claassen 2019") 

df_plot <- df_result_dcpoMean[[1]] %>% 
  mutate(model = "DCPO\n (Point Estimates)") %>%  
  bind_rows(df_plot)
...
plot_clsVsDCPO <- small_multiple(df_plotS) +
  theme_minimal() +
...
plot_clsVsDCPO2 <- small_multiple(df_plotS) +
  theme_minimal() +
  ggtitle("DV: Liberal Democracy (Vdem)")+
  theme(legend.position = "none")

gridExtra::grid.arrange(plot_clsVsDCPO, plot_clsVsDCPO2, ncol = 2)
```

```{r visualizedUncertain, fig.cap= "The Effect of Democratic Mood on Institutional Democratization Accounting for Uncertainty"}

df_plot <- df_result_dcpoMean[[1]] %>% 
  mutate(model = "DCPO\n (Point Estimates)") 
 
df_plot <- df_result_dcpo[[1]] %>%
  mutate(model = "DCPO\n (w. Uncertainty)") %>%
  rename(std.error = se) %>%
  bind_rows(df_plot)
...
save(df_result_clsAPSR, file = here::here("output", "estimates_clsAPSR.RDS"))
```

```{r dcpoPointAPSR}
df_controls <- read_csv(here::here("data", "APSRctlv.csv"),
                        col_types = cols(.default = col_double(),
                                         country = col_character(),
                                         country_name = col_character(),
                                         ISO = col_character(),
...
mods[[2]] <-
  plm(
    diff(theta, lag = 1) ~ plm::lag(theta, 1:2) + diff(Vdem_libdem_z, lag = 1) + plm::lag(Vdem_libdem_z, 1) + diff(lg_imp_mdpgdp, lag = 1) + plm::lag(lg_imp_mdpgdp, 1) + diff(cpi_sd, lag = 1) + plm::lag(cpi_sd, 1),
    data = df_dcpoMeanAPSR,
    model = "pooling"
  )
...
```{r dcpoUncertaintyAPSR, eval = FALSE}
df_dcpo <- purrr::map(1:1000, function(anEntry) {
  dcpo_theta$theta[anEntry,,] %>% 
    reformat_dcpo_output("theta") %>% 
    left_join(reformat_dcpo_output(dcpo_sigma$sigma[anEntry,,], "sigma"),
...
estimate_dcpo <- function(aDataset) {
  
    result_dcpo_1k <- vector(mode = "list")
    
    result_dcpo_1k[[1]] <- plm(diff(theta, lag = 1) ~ plm::lag(theta, 1:2) + diff(Vdem_libdem_z, lag = 1)
    + plm::lag(Vdem_libdem_z, 1) + diff(lg_imp_mdpgdp, lag = 1) + plm::lag(lg_imp_mdpgdp, 1), model = "pooling", data = aDataset)
    result_dcpo_1k[[2]] <- plm(diff(theta, lag = 1) ~ plm::lag(theta, 1:2) + diff(Vdem_libdem_z, lag = 1) + plm::lag(Vdem_libdem_z, 1) + diff(lg_imp_mdpgdp, lag = 1) + plm::lag(lg_imp_mdpgdp, 1) + diff(cpi_sd, lag = 1) + plm::lag(cpi_sd, 1), model = "pooling", data = aDataset)
      

    result_vars_dcpo_1k <- map(result_dcpo_1k, function(aFit){
      mitools::MIextract(aFit, fun = vcovHC_se)
    })

    result_betas_dcpo_1k <- map(result_dcpo_1k, function(aFit){
      mitools::MIextract(aFit, fun = coef)
    })
    
  
  df_result_dcpo <- map2(result_vars_dcpo_1k, result_betas_dcpo_1k, function(aVar, aBeta){
    summary(
      mitools::MIcombine(results = aBeta, variance = aVar)
    ) %>% rownames_to_column(var = "term") %>%
    rename(conf.low = `(lower`,
           conf.high = `upper)`,
           estimate = results) %>%
...
df_result_dcpoAPSR <- map(df_dcpo, estimate_dcpo)

save(df_result_dcpoAPSR, file = here::here("output", "estimates_dcpoAPSR.RDS"))
```

```{r visualizedMood, fig.cap= "Effects of Democracy on Change in Mood"}

load(here::here("output", "estimates_clsAPSR.RDS"))
load(here::here("output", "estimates_dcpoMeanAPSR.RDS"))
...
```{r visualizedSep, fig.cap= "The Effect of Democratic Mood on Institutional Democratization based on DCPO"}
