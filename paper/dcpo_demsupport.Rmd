...
df_result_dcpoMean <- map(ls_mod, function(mod){
  result <- plm(mod, model = "pooling", data = df_dcpoMean)
  df_result <- tidy(result) %>%
    mutate(std.error = vcovBK_se(result))
})

save(df_result_dcpoMean, file = here::here("output", "estimates_dcpoMean.RDS"))
```
...
```{r visualizedMood, fig.cap= "The Effect of Democratic Mood on Institutional Democratization based on DCPO"}
