...
df_result_cls[3:4] <- map(ls_mod, function(mod){
...
save(df_result_cls, file = here::here("output", "estimates_cls.RDS"))
```


```{r dcpo-uncertainty, eval = FALSE}
...
mod_ls <- paste0(
  ...
  " + plm::lag(sigma, 1) + plm::lag(lg_imp_mdpgdp, 1) +
...
df_result_dcpo[3] <- map(mod_ls[[1]], estimate_dcpo, aType = "gmm")

# df_result_dcpo[4] <- map(mod_ls[[2]], estimate_dcpo, aType = "gmm")

save(df_result_dcpo, file = here::here("output", "estimates_dcpo.RDS"))
```


```{r polarizationVisualization, fig.cap="Variance of Polarization"}
...
bootstrapCI_sigma <- function(aData) {
  aSigma <- aData$sigma
  result_temp <- boot(data = aSigma,
                      statistic = Bmean,
                      R = 1000) %>%
    boot.ci(type = "bca")
  tibble(
    mean = mean(aSigma),
    conf.low = result_temp$bca[4],
    conf.high = result_temp$bca[5]
  )
}

df_sigma <- purrr::map(1:1000, function(anEntry) {
  dcpo_sigma$sigma[anEntry, , ] %>%
  reformat_dcpo_output("sigma")
})

df_sigma_us <- map_df(df_sigma, function(aCountry){
  filter(aCountry, country == "United States")
}) 

df_sigma_usCI <- map_df(unique(df_sigma_us$year), function(aYear){
  dataAYear <- filter(df_sigma_us, year == aYear)
  bootstrapCI_sigma(dataAYear)
}) %>% 
  mutate(year = unique(df_sigma_us$year))

df_sigma_2019 <- map_df(df_sigma, function(aCountry){
  filter(aCountry, year == 2019)
})

df_sigma_2019CI <- map_df(unique(df_sigma_2019$country), function(aCountry){
  dataACountry <- filter(df_sigma_2019, country == aCountry)
  bootstrapCI_sigma(dataACountry)
}) %>% 
  mutate(country = unique(df_sigma_2019$country))

plot_us <- ggplot(df_sigma_usCI, aes(x = mean, y = year, xmin = conf.low, xmax = conf.high)) +
  geom_point() +
  geom_errorbarh(height=.2) + 
  scale_y_reverse(breaks = unique(df_sigma_us$year)) +
  xlab(expression("Polarization Estimates, "~sigma)) + 
  ggtitle("Polarization of the U.S. from 1988--2020") +
  theme_minimal() +
  theme(axis.title.y = element_blank())
  

plot_2019 <- ggplot(df_sigma_2019CI, aes(x = mean, y = country, xmin = conf.low, xmax = conf.high)) +
  geom_point() +
  geom_errorbarh(height=.2) + 
  xlab(expression("Polarization Estimates, "~sigma)) + 
  ggtitle("Polarization around the World in 2019") +
  theme_minimal() +
  theme(axis.title.y = element_blank())

gridExtra::grid.arrange(plot_us, plot_2019, nrow = 1)
```
...
load(here::here("output", "estimates_cls.RDS"))

df_plot <- df_result_cls[[1]] %>% 
  mutate(model = "Uncertainty\n Ignored",
         type = "Pooled OLS") 
...
df_plot <- df_result_cls[[3]] %>% 
  mutate(model = "Uncertainty\n Ignored",
         type = "System GMM") %>% 
  bind_rows(df_plot)

df_plot <- df_result_dcpo[[3]] %>% 
  mutate(model = "DCPO",
         type = "System GMM") %>% 
  rename(std.error = se) %>% 
  bind_rows(df_plot) %>% 
  select(term, estimate, std.error, model, type) 
...
plot_pooled <- filter(df_plot, type == "Pooled OLS") %>% 
  dwplot(by_2sd = FALSE) %>% 
  relabel_predictors(
    c(
...
plot_gmm <- filter(df_plot, type == "System GMM") %>% 
  dwplot(by_2sd = FALSE) %>% 
  relabel_predictors(
    c(
`plm::lag(Vdem_libdem, 1:2)1` = "Democracy(t-1)",
...
gridExtra::grid.arrange(plot_pooled, plot_gmm, ncol = 2)
```
