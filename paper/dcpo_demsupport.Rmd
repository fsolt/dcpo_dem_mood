...
Following this line, after replicating the original analyses with point estimates, we take the distribution of the latent (explanatory) variable, the outcome variable, and control variables to account for the uncertainty from the measurement model as well as outcome and control variables.
We take 900 draws from the distribution of public democratic support, duplicate each article's analysis dataset of variables without "measurement error" 900 times, and assign them to each a different random draw from the posterior distribution of democracy indices.
...
In addition to incorporating uncertainty, we expanded Claassen's data pool assembling as much data on democratic support as possible from available opinion surveys around the world. 
...
Besides accounting for uncertainty in latent variable estimates, we also included the measurement uncertainty in democracy indices and corruption index. 
...
And the lowermost set presents the results estimated with the expanded data. 
...
Nor do applying system GMM models and using alternative methods for uncertainty incorporation methods alters the conclusion (see Online Supplementary Material \@ref(fullAJPS), \@ref(fullAPSR), and \@ref(moc)).
...
When the uncertainty is ignored, the coefficient of $\Delta$liberal democracy (the uppermost set), the yearly changes of the institutional feature is negative inferring that democratic mood decreases as democracy increases and vice versa. 
...
result_pooled <- bind_rows(
 ...
 mutate(MOCsumm(mocresult_correctAJPS[["pooled"]]), model = "Corrected"),
...
result_gmm <- bind_rows(
  ...
  mutate(MOCsumm(mocresult_correctAJPS[["gmm"]]), model = "Corrected"),
...
result_pooledAJPS <- bind_rows(result_pooled, result_gmm) %>%
  ...
  mutate(
    ...
    model = factor(model, levels = c("Original", "Corrected", "Uncertainty+Expanded Data")))
...
result_regime <- bind_rows(
  ...
  mutate(MOCsumm(mocresult_correctAJPS[["pooled-regime"]]), model = "Corrected"),
...
result_regimeGmm <- bind_rows(
  ...
  mutate(MOCsumm(mocresult_correctAJPS[["gmm-regime"]]), model = "Corrected"),
...
result_ecm1 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["pooled1"]]), model = "Corrected"),
...
result_fd1 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["fd1"]]), model = "Corrected"),
...
result_ecmReg1 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["pooled-regime1"]]), model = "Corrected"),
...
result_fdReg1 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["fd-regime1"]]), model = "Corrected"),
...
result_ecm2 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["pooled2"]]), model = "Corrected"),
...
result_fd2 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["fd2"]]), model = "Corrected"),
...
result_ecmReg2 <- bind_rows(
 ...
 mutate(MOCsumm2(mocresult_correctAPSR[["pooled-regime2"]]), model = "Corrected"),
...
result_fdReg2 <- bind_rows(
  ...
  mutate(MOCsumm2(mocresult_correctAPSR[["fd-regime2"]]), model = "Corrected"),
