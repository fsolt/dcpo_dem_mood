...
claassen_input_raw1 <- read_csv("data/claassen_input_raw.csv") %>% 
  filter(!(str_detect(item, "army") & # WVS obs identified as problematic by Claassen (note: these are his years right now)
    ...
    (str_detect(item, "strong") &
         ((country=="Egypt" & year==2012) |
              ...
              (country=="Vietnam" & year==2001))))) %>% 
  filter(!(survey=="evs2017" | survey == "arabb5" | survey == "eb471")) %>%  # surveys unused by Claassen
