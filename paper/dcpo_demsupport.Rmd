...
save(result_cls, file = here::here("output", "estimates_clsMeanAJPS.RDS"))
```
...
save(result_clsUncertain, file = here::here("output", "estimates_clsAJPS.RDS"))
```


```{r dcpoPointAJPS, eval = TRUE}
...
tidy_dcpo <- map(df_result_cls, function(result) {
  df_result <- tidy(result, conf.int = TRUE)
...
glance_dcpo <- map(df_result_cls, function(aResult){
  if(class(aResult) == "plm"){ 
    glance.plm(aResult) # using the customized glance function
  }else{
      glance(aResult)
...
save(result_dcpo, file = here::here("output", "estimates_dcpoMeanAJPS.RDS"))
```
...
save(result_dcpoUncertain, file = here::here("output", "estimates_dcpoAJPS.RDS"))
```
