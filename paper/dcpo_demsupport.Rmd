...
Science Review Dataverse: https://doi.org/10.7910/DVN/XAUF3H.
...
if(!require(janitor)) p_install(janitor, force = FALSE)
if(!require(gesisdata)) remotes::install_github("fsolt/gesisdata") # dependent package for DCPOtools
...
# Sys.setenv("DATAVERSE_KEY" = "your_keys") # replace your_keys with true API keys

# Functions preload
set.seed(313)

source(here("customFunctions.R")) ## analyzing and presenting functions
...
# Download from osf if files not present 
# (can also be generated by compiling /data_replication/data_replication.Rmd)
if (!file.exists(here("data",
                      "correct_cls_ajps.rda"))) { 
    osf_retrieve_file("5xyvt") %>% 
        osf_download(here("data"))
}
if (!file.exists(here("data",
                      "correct_cls_apsr.rda"))) { 
    osf_retrieve_file("8xgm4") %>% 
        osf_download(here("data"))
}
if (!file.exists(here("data",
                      "expcor_cls_ajps.rda"))) { 
    osf_retrieve_file("rj9g5") %>% 
        osf_download(here("data"))
}
if (!file.exists(here("data",
                      "expcor_cls_apsr.rda"))) { 
    osf_retrieve_file("bxt7m") %>% 
        osf_download(here("data"))
}
...
load(here("data", "exp_claassen_input.rda")) # https://osf.io/fcyn3/download
...
We proceed in three steps.^[The replication files are available at @TaiEtAl2022a.]
...
```{r compare_n}
# One can download the original data of the AJPS paper with the following codes AFTER set Sys.setenv("DATAVERSE_KEY" = "your_keys")
# if (!file.exists(here("data", "supdem raw survey marginals.tab"))) {
#   dataverse::get_file("supdem raw survey marginals.tab", "10.7910/DVN/HWLW0J") %>%
#     read_csv(col_types = "cdcddcdc") %>%
#     write_csv("data/supdem raw survey marginals.tab")
# }

supdem <- read_csv(here("data", "supdem raw survey marginals.tab"),
                   col_types = "cdcddcdc") %>% 
  janitor::clean_names() %>% 
...
```{r pointAJPS}
# One can download the original data of the AJPS paper with the following codes AFTER set Sys.setenv("DATAVERSE_KEY" = "your_keys")
# if (!file.exists(here("data", "Support_democracy_ajps.csv"))) {
#   get_file_by_doi(filedoi = "10.7910/DVN/HWLW0J/MVXNFC") %>%
#     read_csv(col_types = "cdcddcdc") %>%
#     write_csv(here("data", "Support_democracy_ajps.csv"))
# }
...
saveRDS(result_clsAJPS,
        file = here("output", "estimates_clsMeanAJPS.RDS"))  
```
...
ggsave(here("output","plot_fullAJPS.png"))
```
...
# One can download the original data of the AJPS paper with the following codes AFTER set Sys.setenv("DATAVERSE_KEY" = "your_keys")
# if (!file.exists(here("data", "dem_mood_apsr.RData"))) {
#   get_dataframe_by_doi(filedoi = "10.7910/DVN/FECIO3/ACMGQG") %>%
#     save(file = here("data", "dem_mood_apsr.RData"))
# }
...
saveRDS(mocresult_correctAPSR,
        file = here("output", "estimates_moc_correctAPSR.RDS"))
```
...
saveRDS(mocresult_expAPSR,
        file = here("output", "estimates_moc_expcorAPSR.RDS"))
```
...
ggsave(here("output","plot_fullAPSR.png"))
```
...
Yuehong 'Cassandra' Tai, Yue Hu, and Frederick Solt, "Democracy, Public Support, and Measurement Uncertainty," American Political Science Review.
...
```{r dcpodata-input, include=FALSE}
file.copy(
        c(
            "dcpo_ajps.rda",
            "dcpo_apsr.rda",
...
## Replication data
load(here("data", "dcpo_ajps.rda"))  # https://osf.io/xw9fr/download
load(here("data", "dcpo_apsr.rda"))  # https://osf.io/j7kn3/download
load(here("data", "exp_dcpo_input.rda"))  # https://osf.io/r2pe8/download
...
saveRDS(mocdcpo_APSR,
        file = here("output", "estimates_moc_dcpoAPSR.RDS"))
```
...
Research documentation and data that support the findings of this study are openly available in the APSR Dataverse at https://doi.org/10.7910/DVN/XAUF3H.
