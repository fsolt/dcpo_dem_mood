...
## Facet cause NA column, very weird. 
plot_estimate <- map(c("Pooled OLS", "System GMM"), function(aType) {
  filter(df_plot, type == aType) %>%
    dwplot(by_2sd = FALSE) %>%
    relabel_predictors(
      c(
...
library(ggpubr)
ggarrange(plotlist = plot_estimate, common.legend = TRUE)
