...
```{r visualizeAJPS, fig.cap= "The Effect of Public Support on Democracy", eval = TRUE, fig.width=7.5, fig.height=10}
...
```{r visualizedAPSR, fig.cap= "The Effect of Democracy on Public Support", eval = TRUE, fig.width=7, fig.height=10}
...
```{r visualizeFullUncertainAJPS, fig.cap= "The Effect of Public Support on Democracy", eval = TRUE, fig.width=7.5, fig.height=7}
...
```{r visualizedFullUncertainAPSR, fig.cap= "The Effect of Democracy on Public Support", eval = TRUE, fig.width=7, fig.height=10}
...
saveRDS(result_clsUncertain_whole, file ="output/clsAJPS_uncertainty.RDS")

```

```{r MocdcpoUncertaintyAJPS}
load("data/dcpo_ajps.rda")
...
saveRDS(result_dcpoUncertain_whole, file ="output/dcpoAJPS_uncertainty.RDS")
...
result_pooled <- bind_rows(
  ...
  mutate(MOCsumm(result_dcpoUncertain_whole[[1]]), model = "DCPO")
) %>% 
  mutate(type = "Pooled")
...
result_regime <- bind_rows(
  ...
  mutate(MOCsumm(result_dcpoUncertain_whole[[2]]), model = "DCPO")
) %>% 
  mutate(type = "Pooled")
...
saveRDS(result_clsUncertain, file = "ouput/clsAPSR_uncertainty.RDS")
...
saveRDS(result_dcpoUncertain, file ="output/dcpoAPSR_uncertainty.RDS")
