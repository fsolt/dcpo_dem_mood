...
When we recollected the original data, we could not entirely achieve a primary replication [see @Grossman2020, p. 18] based @Claassen2020a's Dataverse materials.
It turned out that we have differences in number of country-year surveys and in sample size.
Specificially, our replication data has five fewer observations, three of which are no longer available in originial survey data and 2 seem to have never existed, and one more survey item which was conflated in original data [See more detail in Appendix].
...
Following this line, after replicating the original analyses with point estimates, we incorportate uncertainty in analysis model using our replication data.
We take the distribution of the latent (explanatory) variable, the outcome variable, and control variables to account for the uncertainty from the measurement model as well as outcome and control variables.
...
Besides accounting for uncertainty in latent variable estimates, we also included the measurement uncertainty in democracy indices and corruption index [See Appendix].
...
*Personally, I think we should be careful to use MO since our approach is not MO. I will go back here later. Instead, we might just write down Rubin's rule here. * 
