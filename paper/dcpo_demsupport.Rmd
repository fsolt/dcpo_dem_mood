...
Similarly, studies have alternately found that more experience with democracy yields more democratic support [e.g., @FailsPierce2010, 183] or instead that long-established democracies are suffering from democratic fatigue [e.g., @FoaMounk2017].
...
They point to a need for closer attention to the conditional aspects of the classic theory [see @Lipset1959a, 86-89; @Easton1965, 119-20].
...
This allows MOC to be broadly applicable adapt to many different types of models, including time-series-cross-sectional models [@CaugheyWarshaw2018a, A15-16], Cox proportional hazards models [@TreierJackman2008, 215], or models of individual-level roll call voting [@KastellecEtAl2015, 791].^[For additional details on the MOC technique, see  OSM \@ref(moc).]
...
We employed `r dcpo_input_ncyi` national opinions on democracy from `r dcpo_input_surv` national surveys, representing a `r increase_cyi`% and `r increase_surv`% increase respectively over the `r supdem_ncy` opinions and `r supdem_surv` national surveys used in Claassen [-@Claassen2020a; -@Claassen2020].^[
...
In Figure \@ref(fig:plot-fullAPSR), we examine the thermostatic model of democratic support per @Claassen2020 [47, Table 1, 49, Table 2].
...
As @CaugheyWarshaw2018a [A-15] explained, the main idea of MOC is to estimate the marginal distribution of coefficient parameter vector $\beta$, integrating over the uncertainty in latent variables $\theta$.
...
In this way, we incorporate uncertainty in measuring predictor $\theta$, and uncertainty in the effects of latent variables $\theta$ and other variables $w$ on outcome variable $y$ [@TreierJackman2008, 215].

To sample from the conditional density and the marginal density in the right side of the equation, we follow iterative Monte Carlo procedure described by @TreierJackman2008, at iteration $t$,
...
Third, to produce more sensible estimates of uncertainty for observations at the extremes of the scale [see @LinzerStaton2015a, 229], the DCPO model places bounds on its estimates of public opinion [@Solt2020a, 8].
