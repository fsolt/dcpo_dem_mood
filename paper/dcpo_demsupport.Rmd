...
tidy_result_clsUncertain <- map(ls_mod, function(aMod){ # reading a model
  ...
  result_vars <- ifelse(class(result[[1]])[1] == "plm", 
                        eval(mitools::MIextract(result, fun = vcovBK_se)),
                        eval(mitools::MIextract(result, fun = vcovHC_se)))
...
save(tidy_result_clsUncertain, file = here::here("output", "estimates_clsAJPS.RDS"))
```
