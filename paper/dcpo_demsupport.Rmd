...
In addition to incorporating the measurement uncertainty, we also sought to reduce it by expanding the estimated data to including 144 countries for up to 33 years between 1988 and 2020 (based on 26% more questions and 33% more survey sources).
...
As the third step, we collect even more survey data---increasing these source data observations by one-fourth---and re-estimate the two articles' analyses once more, again maintaining the full distribution of estimates to preserve measurement uncertainty.
...
Following this line, after replicating the original analyses with point estimates, we incorporate the full distribution of the latent and control variables to the estimation.
We follows the 
...
Then we perform each analysis repeatedly on its respective collection of the datasets, and combine the results following the rules set out in @Rubin1987 [see, e.g., @Schnakenberg2014; @Crabtree2015].
This uncertainty-incorporation process is analogous to @Blackwell2017a's multiple overimputation (MO).
We also employ one alternative technique known as “method of composition” [@treier_democracy_2008, p. 215] as a robustness check [see the Online Supplementary Material \@ref(moc)].
...
A noteworthy point is that MO method enables not only uncertainty incorporation of key explanatory and outcome variables but also control variables. 
We fully utilize this strength to include the uncertainty of the measurement of
democracy and corruption. 
[XXXX Cassandra, please fill in a tiny little bit of details here]
...
The estimation based on an alternative method of uncertainty incorporation, model of composition, yields identical conclusions [XXXX].
...
With MO, we draw variable values from their distributions and estimate the coefficient first before combining through Rubin's rule.
The deviation of the result from the original indicates the coefficient (point) estimates were not that consistent.
...
\begin{equation}
\mathcal{L}(\theta|y, x^{obs}) \propto \int\limit_{\chi} p(x|y, \theta)p(y|\theta)dx^{mis}.
\end{equation}
...
\begin{equation}
\mathcal{L}(\theta|y, x) \propto \int\limit_{\chi} p(\theta|y, x)p(x|w)dx.
\end{equation}
...
```{r plot-mocAPSR, fig.cap= "The Effect of Democracy on Public Support", eval = TRUE, fig.width=7, fig.height=10}
