...
Public support for democracy is a latent variable and the incorrect measurement of it may conceal the real relationship between public opinion and democratic institution development.
...
We use a superior measure, Dynamic Comparative Public Opinion (DCPO) model developed by@Solt2020a, to estimate democratic support for a larger dataset with 144 countries for up to 33 years between 1988 and 2020.
...
Omitting to account for measurement error distorts both statistical and substantive inference (See Juhl 2019for review in this literature). 
...
However, in comparative public opinion field, previous efforts on measuring comparative public opinion didn’t account for measurement uncertainty in their analysis.  
To address the uncertainty, the inherent trail of latent constructs, in addition to employ the DCPO which reduces uncertainty in measurement, we also incorporated uncertainty in our analysis by using random draws from the posterior distributions and following @Rubin1987's rules. 
...
In addition to methodological concerns, our reanalysis echoes recent studies about public’s behavioral commitment to democracy.  
It is questionable that whether self-report based conventional measures of support for democracy could transfer to behaviorally democratic commitments when the democratic system is in economic or societal crisis.
...
For example, our data consists of 20885 national responses as the result of measuring public attitude at ordinal scales, which has potential to give us more nuances about public demcratic support. 
...
This approach allows us to keep information embeded in questions rather than discarding them directly. 
In sum, the DCPO model has methodological advantages in keeping as much as available information, reducing uncertainty by bounding scales and capturing the extent of polarization. 
The validation tests in @Solt2020a's paper corroborate that DCPO not only features the smallest mean absolute error(MAE) and largest improvement over the country-means MAE but also provides a more stable and efficient prediction on out-of-sample data than either of @Claassen2019 Model 5 and @Caughey2019 model.
Following @Solt2020a's apporoach, our validation test employ k-fold validation with 10 folds, which reduces the misrepresentation of the model due to the chosen data from single cross-validation (See details of the validation test in Appendix).
...
The latent constructs are inherently accompanied with amount of measurement uncertainty due to the unobservability of variables.
The measurement is more far away from prefection when concepts are complex. 
With a strong assumption that latent variables are measured perfectly, the omittance to incorporate measurement error might obscure the true relationship between theoretical concepts and invalidates findings.
...
In our reexamination, instead of employing the point estimates, we relax the implausible assumption of perfect measurment of latent variables, we followed the recommendation and the approach of incorporating uncertainty in using latent variables [e.g., @Schnakenberg2014; @Crabtree2015] and other measures, such as the Standardized World Income Inequality Database [@Solt2020].
...
# Results
Our reanalysis follows the following steps. Frist, we replicated Claassen's  (2019,2020)  studies by using his measurement and analysis models.
Results here.
Next, adding measurement uncertainty into Claassen's analysis models, we found xxxx.
Tables and results here.
We then reanalyzed the relationship between public democratic attitude and democracy institution in both directions, using more data, the DCPO measurement model and measurement uncertainty incorporated.
Results here. indistinguishable from zero.
...
Second, these survey items do capture public democratic support, but we still don’t have enough data to compensate the fragmentation and incomparability of the available comparative public opinion data, which is doomed to accompany with high uncertainty.
...
After the Cold War, many democracies backslided and imperiled in the worldwide because elected leaders lack of commitment to democratic norms when the public even did not notice that (Bermeo 2016; Levtsky and Ziblatt 2018, Waldner and Lust 2018).
...
A recent study shown that polarized public cannot get a consensus on democratic erosion which experts preserved (Carey et al 2018).
Given the measures for democratic development are from experts, there could be a gap between public perception on democracy and the measures of the levels of democracy.
...
```{r visualizedMoodAJPS, fig.cap= "The Effect of Democratic Mood on Institutional Democratization"}
...
```{r visualizedMoodAPSR, fig.cap= "Effects of Democracy on Change in Mood"}
