...
Measurement error challenges statistical analyses, especially when it is correlated with a dependent variable which is the case of democratic support, but it is often ignored in the analysis [@Blackwell2017; @Blackwell2017a].
...
Especially when measurement error is correlated with the dependent variable, the resulting bias could either attentuate or exaggerate the magnitude of the estimated effects [see, e.g., @Blackwell2017, 360].
...
Our approach of incorporating uncertainty is similar in the logic of @Blackwell2017a's multiple overimputation, that is, creating multiple data set from the posterior distribution of the true latent value, applying analysis model for each dataset, and then averaging results by using Rubin's rule. 
...
result_pureAPSR <- map(ls_mod_apsrCLS, function(aMod){ # reading a model
  result <- map(pure_cls_apsr, function(aData){ # applying to the 1000 datasets
    eval(parse(text = aMod))
  })
...
```{r plot-mainAPSR, fig.cap= "The Effect of Democracy on Public Support", fig.width=7, fig.height=10, eval = TRUE}
...
type_model <-
  c("Claassen 2020", "Claassen Uncertainty", "DCPO Uncertainty")
...
dwplot(df_plot) +
    geom_vline(xintercept = 0,
               ...
               linetype = 2) +
    theme_minimal() +
    ggtitle("DV: Public Support for Democracy") +
    xlab("Coefficient Estimate") +
    theme(
        axis.title.y = element_blank(),
        legend.position = "bottom",
...
result_dcpoUncertain <- map(ls_mod_ajpsCLS, function(aMod){ # reading a model
...
result_expAPSR <- map(ls_mod_apsrDCPO, function(aMod){ # reading a model
...
saveRDS(result_expAPSR, file = here::here("output", "estimates_dcpoAPSR.RDS"))
```


# Replications of Full Pack of Models in Claassen 2020a,b

```{r plot-fullpackAJPS, fig.cap= "The Effect of Public Support on Democracy", fig.width=7.5, fig.height=10, eval=TRUE}
...
```{r plot-fullpackAJPSfull, fig.cap= "The Effect of Public Support on Democracy", eval = TRUE, fig.width=7.5, fig.height=7, eval=TRUE}
...
result_pooled <- bind_rows(
  mutate(result_cls$pooled[[1]], model = "Claassen 2020a"),
  mutate(result_pureAJPS$pooled[[1]], model = "Claassen w/ Uncertainty"),
  mutate(result_expAJPS$pooled[[1]], model = "DCPO")
) %>% 
  mutate(type = "Pooled")

result_gmm <- bind_rows(
  mutate(result_cls$gmm[[1]], model = "Claassen 2020a"),
  mutate(result_pureAJPS$gmm[[1]], model = "Claassen w/ Uncertainty"),
  mutate(result_expAJPS$gmm[[1]], model = "DCPO")
) %>% 
  mutate(type = "GMM")
...
result_regime <- bind_rows(
  mutate(result_cls$`pooled-regime`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAJPS$`pooled-regime`[[1]], model = "Claassen w/ Uncertainty"),
  mutate(result_expAJPS$`pooled-regime`[[1]], model = "DCPO")
) %>% 
  mutate(type = "Pooled")

result_regimeGmm <- bind_rows(
  mutate(result_cls$`gmm-regime`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAJPS$`gmm-regime`[[1]], model = "Claassen w/ Uncertainty"),
  mutate(result_expAJPS$`gmm-regime`[[1]], model = "DCPO")
) %>% 
  mutate(type = "GMM")
...
```{r plot-fullpackAPSR, fig.cap= "The Effect of Democratic Mood on Institutional Democratization based on DCPO", fig.width=7.5, fig.height=10, eval=TRUE}

result_clsAPSR <- readRDS(here("output", "estimates_clsMeanAPSR.RDS"))
result_clsUncertainAPSR <- readRDS(here("output", "estimates_clsAPSR.RDS"))
result_expAPSR <- readRDS(here("output", "estimates_dcpoAPSR.RDS"))
...
result_ecm1 <- bind_rows(
  ...
  mutate(result_expAPSR$pooled1[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fd1 <- bind_rows(
  ...
  mutate(result_expAPSR$fd1[[1]], model = "DCPO")
) %>% 
...
result_ecm2 <- bind_rows(
  ...
  mutate(result_expAPSR$pooled2[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fd2 <- bind_rows(
  ...
  mutate(result_expAPSR$fd2[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
result_ecmReg1 <- bind_rows(
  ...
  mutate(result_expAPSR$`pooled-regime1`[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fdReg1 <- bind_rows(
  ...
  mutate(result_expAPSR$`fd-regime1`[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
result_ecmReg2 <- bind_rows(
  ...
  mutate(result_expAPSR$`pooled-regime2`[[1]], model = "DCPO")
) %>% 
  mutate(type = "ECM")
...
result_fdReg2 <- bind_rows(
  ...
  mutate(result_expAPSR$`fd-regime2`[[1]], model = "DCPO")
) %>% 
  mutate(type = "First Difference")
...
```{r plot-fullpackAPSRfull, fig.cap= "The Effect of Democracy on Public Support", eval = TRUE, fig.width=7, fig.height=10, eval=TRUE}

result_clsAPSR <- readRDS(here("output", "estimates_clsMeanAPSR.RDS"))
result_pureAPSR <- readRDS(here("output", "estimates_clsFullUncertainAPSR.RDS"))
result_dcpoFullUncertainAPSR <- readRDS(here("output", "estimates_clsExpAPSR.RDS"))

# Regime compacted ####

result_ecm1 <- bind_rows(
  mutate(result_clsAPSR$pooled1[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$pooled1[[1]], model = "Claassen w/ Uncertainty"),
...
result_fd1 <- bind_rows(
  mutate(result_clsAPSR$fd1[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$fd1[[1]], model = "Claassen w/ Uncertainty"),
...
result_ecm2 <- bind_rows(
  mutate(result_clsAPSR$pooled2[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$pooled2[[1]], model = "Claassen w/ Uncertainty"),
...
result_fd2 <- bind_rows(
  mutate(result_clsAPSR$fd2[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$fd2[[1]], model = "Claassen w/ Uncertainty"),
...
result_ecmReg1 <- bind_rows(
  mutate(result_clsAPSR$`pooled-regime1`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$`pooled-regime1`[[1]], model = "Claassen w/ Uncertainty"),
...
result_fdReg1 <- bind_rows(
  mutate(result_clsAPSR$`fd-regime1`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$`fd-regime1`[[1]], model = "Claassen w/ Uncertainty"),
...
result_ecmReg2 <- bind_rows(
  mutate(result_clsAPSR$`pooled-regime2`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$`pooled-regime2`[[1]], model = "Claassen w/ Uncertainty"),
...
result_fdReg2 <- bind_rows(
  mutate(result_clsAPSR$`fd-regime2`[[1]], model = "Claassen 2020a"),
  mutate(result_pureAPSR$`fd-regime2`[[1]], model = "Claassen w/ Uncertainty"),
...
ls_iv <- c("plm::lag(SupDem_trim, 1)",                         
           "plm::lag(theta_dem_trim, 1) + plm::lag(theta_aut_trim, 1) ") ##all use trim
ls_method <- c("plm", "pgmm")
...
df_dcpo <- purrr::map(1:900, function(anEntry) {
  exp_cls_ajps[[anEntry]] %>%     #original data has include theta_dem_trim
    plm::pdata.frame(index = c("country", "year"))
})


ls_iv <- c("plm::lag(theta_trim, 1)", 
           "plm::lag(theta_dem_trim, 1) + plm::lag(theta_aut_trim, 1)") #use trimed data
ls_method <- c("plm", "pgmm")
...
load(here("data", "pure_cls_apsr.rda")) #use pure_cls_apsr instead of cls_apsr
load(here("data", "exp_cls_apsr.rda")) #use exp_cls_apsr instead of cls_apsr
...
df_our_clsAPSR <- purrr::map(1:900, function(anEntry) {
...
result_our_clsUncertain  <- map(ls_mod, function(aMod){
  result <- methodComposition3(df_our_clsAPSR,aMod)
})   ##our expanded data

names(result_clsUncertain) <- c("pooled1", "pooled-regime1", "pooled2", "pooled-regime2",
                                "fd1", "fd-regime1", "fd2", "fd-regime2")

names(result_our_clsUncertain) <- c("pooled1", "pooled-regime1", "pooled2", "pooled-regime2",
                                "fd1", "fd-regime1", "fd2", "fd-regime2")

saveRDS(result_clsUncertain, file = here::here("ouput","clsAPSR_uncertainty.RDS"))

saveRDS(result_our_clsUncertain, file = here::here("ouput","our_clsAPSR_uncertainty.RDS"))
...
result_clsUncertain <- readRDS(here("output","our_clsAPSR_uncertainty.RDS"))
