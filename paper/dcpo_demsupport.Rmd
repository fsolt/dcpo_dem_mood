...
save(dcpo_input, dcpo_x_pred, file = here::here("data", "dcpo_y_r_pred.rda"))
```
...
load(here::here("data", "dcpo_input_update.rda"))
load(here::here("data", "dcpo_theta.rda"))

ls_year <- 1988:2017
ls_country <- dcpo_input$data$country %>% 
  unique
...
result_dcpo_1k <- map(df_theta, function(aDataset) {
  plm(
    Vdem_libdem ~ plm::lag(Vdem_libdem, 1:2) + plm::lag(theta, 1) + plm::lag(lg_imp_mdpgdp, 1) + plm::lag(mdprgdp_grwth, 1) + plm::lag(Region_UN, 1) + plm::lag(muslism_prop_2010, 1) + plm::lag(dependence_pc_di, 1),
    model = "pooling",
    data = aDataset
  )
})
...
summary(mitools::MIcombine(results = result_betas_dcpo_1k, 
