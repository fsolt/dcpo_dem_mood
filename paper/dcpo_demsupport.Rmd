...
tidy_result_cls <- map(df_result_cls, function(result) {
...
glance_result_cls <- map(df_result_cls, function(aResult){
...
names(tidy_result_cls) <- names(glance_result_cls) <- c("pooled", "pooled-regime", "gmm", "gmm-regime")

save(tidy_result_cls, glance_result_cls, file = here::here("output", "estimates_clsMeanAJPS.RDA"))
```
...
tidy_result_clsUncertain <- map(ls_mod, function(aMod){ # reading a model
  ...
  summary(mitools::MIcombine(results = result_betas, 
...
save(tidy_result_clsUncertain, file = "estimates_clsAJPS.RDS")
```


```{r dcpoPointAJPS}
# Not showing in the final result
...
tidy_result_dcpoUncertain <- map(ls_mod, function(aMod){ # reading a model
...
save(tidy_result_dcpoUncertain, file = here::here("output", "estimates_dcpoAJPS.RDS"))
```
...
load(here::here("output", "estimates_clsMeanAJPS.RDA"))
...
ls_mods[["FD1.1"]] <-
  plm(
    SupDem_trim ~ Libdem_z + lnGDP_imp,
    model = "fd",
    data = sd.plm,
    index = "Country"
  )
...
ls_mods[["FD1.2"]] <-
  plm(
    SupDem_trim ~ Libdem_z + lnGDP_imp + Corrup_TI_z,
    model = "fd",
    data = sd.plm,
    index = "Country"
  )
...
ls_mods[["ECM2.1"]] <-
  plm(
    diff(SupDem_trim, lag = 1) ~ plm::lag(SupDem_trim, 1:2) + diff(Polyarchy_z, lag = 1)
...
ls_mods[["FD2.1"]] <-
  plm(
    SupDem_trim ~ Polyarchy_z + Liberal_z + lnGDP_imp,
    model = "fd",
    data = sd.plm,
    index = "Country"
  )
ls_mods[["FD2.2"]] <-
  plm(
    SupDem_trim ~ Polyarchy_z + Liberal_z + lnGDP_imp + Corrup_TI_z,
    model = "fd",
    data = sd.plm,
    index = "Country"
  )
...
df_result_clsAPSR <- map(ls_mods, function(aMod){
...
ls_result_cls <-
  map2(tidy_result_cls, glance_result_cls, function(aTidy, aGlance) {
    ls_sum <- list(tidy = aTidy,
                   glance = aGlance)
    
    class(ls_sum) <- "modelsummary_list"
    
    return(ls_sum)
  })

names(ls_result_cls) <- c("Pooled OLS 1", "Pooled OLS 2", "GMM 1", "GMM 2")
