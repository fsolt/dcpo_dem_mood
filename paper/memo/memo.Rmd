...
Both methods yield the similar results in significant and magnitude, though they address measurement errors in different angles. 
...
It is possbile that the relationship is negligible. 
The incorporation of measurement error swamping the significant effects of democracy in a strong relationship is rare [@Treier2008].
...
Even in our expanded data, xx data is missing, compared to xxx missing in original data, which might cause the domination of priors in smoothing over-time changes.
However, in our revised manuscript, we only employed Claassen's Model 5 to estimate public support using corrected original data and expanded data, seperately.
Therefore, we isolate the effect of priors on estimates. 
...
The results render strong evidence that the coefficient estimates from expanded data are more certainty with narrow confident interval.
However, since the temporal evoluation of latent public support is specificed as a simple dynamic linear model and variance of noise term is constant across countries, the actual changes in mood might not be accurately captured instead being smoothed over. 
That might explain why literally there is nothing significant when uncertainty is accounted in APSR replication. 
