...
Many variables, dependent variable, key independent variables and control variables in original analysis, are latent variables, which cannot be observed directly and are inherently contaminated by measurement errors.
To address this concern and justify the necessity of incorporating measurement uncertainty in latent variables, we take uncertainty into account for five variables, public support for democracy, liberal democracy index, electoral democracy index, liberal component index, and corruption perception index, in this version of manuscript.

In respect of the key independent variable, public support for democracy, we used Claassen's Model 5 and recollected originalsource data to replicate the measure of public support and keep its posterior distrubtion.
For expanded data, still, we used Model 5 and expanded source data to measure public support and perserve its posterior distribution. 
...
However, since CPI does not povide standard error for the years before 2012, we first estimated the standard error for these years by using the product of countries' maximum relative error (standard error/cpi index) and their CPI index, and then used the estimated standard error to simulate 900 draws. 

We used he posterior distributions of five variables in method of composition to address measurement errors. 
...
The results from models including measurement error for outcome, key independent and control variables again justify the importance of incorporating uncertainty in analysis.
...
The null results hold in robust check models and models with more control variables in Figure xx Page xx in Online Supplementary material.
...
A bigger size of observed data is expected to reduce the possible dominance of priors.
However, even though we put efforts to collect as much data as possible.
The expanded data is only xxx county-year.
It is possible that the changes of public mood are "smooth" out by modeling the temporal evolution in a simple local-level dynamic linear.
The smoothed time series data might fail to capture a sudden change which is caused by exogenous factors.
...
Many studies have already shown that when taking measurement uncertainty into account, the magnitude of coefficients of latent variables can be either attenuated or enlarged substantively [e.g., @Treier2008, p. 211; @caughey2018,p. 254].
...
See Page xxx, Figure xxx, and xxx in SI. 
...
In replications for both AJPS and APSR analyses, it is uncertainty that accounts for the disappearing significance of public support and changes in democracy. 
In Supplementatry material, we explained the methodology for addressing measurement uncerainty in this study and compared Rubin's Rules and Method of Composition.
Both methods yield the similar results in significant, though they address measurement errors in different angles. 

In terms of disappearing relationships between public support and democracy and the change in mood and the change in democracy, we have discussed theoretical reasons in our main text. 
It is possbile that the relationship is negligible, because the incorporation of measurement error swamping the significant effects of democracy in a strong relationship is rare [@Treier2008].

In addition to potential methodological explanations we have discussed, another potential reason is the over-time smoothing induced by the random-walk prior thanks to Reviwer 2's suggestion.
At aggregated level, country-year data is super sparse. 
Even in our expanded data, xx data is missing, compared to xxx missing in original data.
The sparse of raw data and the simple dynamic linear model used in measurement model to capture dynamic effect might make priors dominate in smoothing over-time changes. 
However, in our revised manuscript, we only used Claassen's Model 5 to estimate public support using corrected original data and expanded data, seperately.
Since our expanded data added xxx country-year, the estimates from the expanded data should be influenced less by priors than original data by inputing more data. 
Still, the coefficient estimates from expanded data are indistinguishable from zero though with narrower confident interval.
Therefore, it is the incorporation of uncertainty that changes the coefficient estimates in magnitude and significant. 
<!--random walk,standards for dv -->
...
## Response
In SI, we discussed methodology used in this article to address measurement errors, Rubin's Rules and MOC.
We compared these two methods by introducing their fomulas and equations and highlighing their different approaches. 
As results show, though they have different approaches, the results from two methods are similar with each other. 
