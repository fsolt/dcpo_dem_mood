...
## Response 
Both Reviewers 1 & 2 concern about the validity of the approach which we used to incorporate measurement uncertainty. First, we added more explanations on how uncertainty is integrated into the models in Page xx of main text and also presented equations in Page xx in SI. It is ture that Rubin's rule is designed to address missing values by using multiple imputation(MI), but as Reviewer 2 has already pointed out, @Blackwell2017 applied the logic of MI and Rubin's rule in their Multiple Overimputation (P309) to address measurement error and missing data which is conceptulized as an extreme case of measurement error. 

The idea of our approach for propagating uncertainty is correcting both within and between imputation variance through Rubin's rule, and the idea of MOC is accounting for within imputation variance through drawing sample from the joint posterior distribution of the measurement and analysis model. 

To address the performance of the method we used, we employed "method of component" as an alternative way in SI.
...
In terms of the impact of measurement error on estimated effects, many studies have shown when taking measurement uncertainty into account, magnitude of coeffients of latent variables can be either attenuated or enlarged substantively [e.g., @Treier2008, p. 211; @caughey2018,p. 254].
...
## Response
First, we need to clarify the DV in Figure2 is the change of public support for democracy rather than current public support.
Second, when DV is the change of public support, Claassen's original results wouldn't make any sense. 
Claassen's original results indicate the change of public support in one year is positively changed by the public support in the previous one year and negatively shifted by the previous two years.
However, should we expect that public support can produce its demand and especially a conflicting demand?
Can high public support of this year ask for a change in next year but resist a change in the next two years?
In the contrast, our null results of lagged support on change of support mood make more sense since the change of moods is supposed to be shaped by exogenous factors, such as democratic or economic environment, or especially a shock.
...
As @Treier2008 emphasized, only in applications where democracy has a strong effect on the quantity of interest and models are simple enough, it is rare that accounting of uncertainty renders the insignificant effect of democracy.
However, even in Claassen's original results, the estimated effects of both the change of democracy and other social as well as economic factors are small.
Given the measurement errors in both democratic indices and public support, it is not surprising to get the diminution of the estimated effects.
...
## Response
Since we do acknowledge uncertainty in extant measurements, we can not unambigously make a statement that there is no relationship between public support and regime survival although it is possible.
Examing theories in different contexts overtime is a promising strategy for generalization [KKV].
However, scholars' pursue on models usually lead to the ignorance for measurements which cost more time but determine the quality of data in analyses [@stegmueller2011].
In this letter, we stress the necessity of incorporating measurement uncertainty in analyses with latent variables and encourage scholars to pay more attention on that.
...
We also elaborate the imputation process in details in MS and clarify the equations in SI.
Measurement errors in other variables have been included in our rexaminations. 
As for the concerns on Fig2, the first thing needing to clarify is that the DV is not current public support but the change of public support. 
We elaborate why our null results make more sense than Claassen's original results. 
...
For example, standard errors did not change after accounting for uncerainty in [@caughey2018] but became smaller in [@Treier2008].
Second, the propogation of uncertainty can either attentuate or magnify the magnitude of estimated effects.
*We can cite more lit if neccessary*. 
For transparcy and avoiding confusion, we only focus on uncertainty in revised version. 
